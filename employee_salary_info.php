...
<main class="page-content">
	...
	<div class="containter-fluid">
					...
					<div class="" style= "margin:0px auto; background:#eee; border:
						...
						<?php 
							...
							if(isset($_POST['submitsalary'])){
								...
								`personal_pay`, `fixed_pay`, `over_time`, 
								`deduct_one_day`, `pf_contribution`, `food_from_sc`, `status`)  values 
								('$department_id','$employee_name','$grade_id','$basic_salary',
								'$personal_pay','$fixed_pay','$overtime',
								'$oneday_food_deduct','$provident_found','$food_deduct','$status')";
								
								// Insert into Service Charge Table 
								if($service_charge>0){
								$query2="INSERT INTO `service_charge_info`
								(`department_id`, `em_id`, `service_charge`, `status`)values
								('$department_id','$employee_name','$service_charge','$status')";
								 $result2=$db->insert($query2);
								}else{
									$query2="INSERT INTO `service_charge_info`
									(`department_id`, `em_id`, `service_charge`, `status`)values
									('$department_id','$employee_name','$service_charge','0')";
									 $result2=$db->insert($query2);
								}
								$results=$db->insert($query);
								
								
								if($results & $result2 ==true){
									?> 
							...
							<div class="col-sm-12 form-group">
								...
								<select id="grade" name="grade" class="form-control browser-default custom-select">
									...
									if ($results4==true){
											...
											$id++; 
									?> 
									<option value="<?php echo $res4['id']; ?>"><?php echo $res4['grade_name']; ?></option>
									<?php }} ?>		
		...
		<div class="col-md-5">
			<div class="row">
				<!--salary information upload part--> 
					<div class="col-md-12">
							...
							if(isset($_POST['importSubmit'])){ 
								...
								if($_FILES["file"]["size"]> 0){
									...
									while(($column=fgetcsv($file, 10000, ",")) !== FALSE){
										  ...
										  $query ="INSERT INTO `salary_details_info`(`department_id`, `employee_id`, `grade_id`, 
										  `basic_salary`, `personal_pay`, `fixed_pay`, 
										  `service_charge`, `over_time`, `deduct_one_day`, 
										  `pf_contribution`, `food_from_sc`, `status`) values 
						...
						<h5> Salary Details Information Upload</h5>
						<hr />
							<form action="employee_salary_info.php" method="post" enctype="multipart/form-data">
								<div class="col-sm-12 form-group">
